...
{
	res.redirect(301, '/logs' + (req.params ? req.params[0] : ''));
	res.end();
}
...
function logs_common(args, req, res)
{
	...
	data.getRecentMessageList(null, args, function (err, results)
	{
		...
		res.end(Plates.bind(String(g_server.getStatic('./pages/logs.html')), {'messages': results}, map));
	});
}
...
{
	logs_common({game: req.params.id_game}, req, res);
}
