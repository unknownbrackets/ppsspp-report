...
function report_compat(req, res)
{
	...
	promise.then(() => {
		...
		if (params.suggestions)
			suggestions = suggest.query(params);
...
function game_info(req, res)
{
	...
	async.auto({
		...
		configs: ['reports', function (args, cb)
		{
		...
		icon: ['game', function (args, cb)
		{
			uploadCache.iconExists(args.game.id_game, cb);
		}],
		screenshots: ['game', function (args, cb)
		{
			uploadCache.getScreenshots(args.game.id_game, cb);
		}],
		version_icons: ['versions', function (args, cb)
		{
			async.map(args.versions, function (g, cb)
			{
				uploadCache.iconExists(g.id_game, cb);
			}, cb);
		}],
	}, function (err, data)
	{
