...
module.exports.getCount = async function (parameters)
{
	...
	if (parameters.family === 'yes')
		clauses.push('EXISTS (SELECT gf.id_game FROM game_families AS gf WHERE gf.id_game = g.id_game)');
	else if (parameters.family === 'no')
		clauses.push('NOT EXISTS (SELECT gf.id_game FROM game_families AS gf WHERE gf.id_game = g.id_game)');
