...
async function checkExisting(params)
{
	const result = await async.auto({
		...
		id_game: lookups.getGameId.bind(this, params.game, params.game_title, params.module_name, params.module_crc),
		...
		configs: [
			...
			async function (args)
			{
				...
				return pool.query(`
					SELECT cv.id_config, cs.title, cd.value
					FROM config_values AS cv
						INNER JOIN config_settings AS cs USING (id_config_setting)
						INNER JOIN config_data AS cd USING (id_config_data)
