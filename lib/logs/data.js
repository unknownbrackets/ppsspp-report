...
module.exports.getRecentMessageList = function (args, cb)
{
	...
	{
		clauses.push('mp.id_platform = ' + mysql.escape(args.id_platform));
		needsTemp = true;
	...
	{
		clauses.push('mc.id_cpu = ' + mysql.escape(args.id_cpu));
		needsTemp = true;
		needsTempDistinct = true;
	...
	{
		clauses.push('mg.id_gpu = ' + mysql.escape(args.id_gpu));
		needsTemp = true;
		needsTempDistinct = true;
		...
		var actualQuery = function (queryArgs)
		{
			conn.query('\
				SELECT \
					...
					GROUP_CONCAT(p.title SEPARATOR \', \') AS platforms, \
					GROUP_CONCAT(cpus.summary SEPARATOR \', \') AS cpus, \
					GROUP_CONCAT(gpus.short_desc SEPARATOR \', \') AS gpus, \
				...
				FROM report_messages AS m' + extraJoinSQL + ' \
					...
					INNER JOIN report_message_cpus AS mc USING (id_msg) \
					INNER JOIN cpus USING (id_cpu) \
					INNER JOIN report_message_gpus AS mg USING (id_msg) \
					INNER JOIN gpus USING (id_gpu) \
				' + makeWhere() + ' \
